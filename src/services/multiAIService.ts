...
export class MultiAIService {
  ...
  private apiType: 'openai' | 'dashscope' | 'baidu' | 'zhipu' | 'deepseek';
  ...
  async generateScript(productInfo: ProductInfo, templateType: ScriptTemplateType): Promise<ScriptContent> {
    try {
      ...
      console.log(`使用 ${this.apiType} API 生成脚本`);
      ...
      console.error(`${this.apiType} API调用失败:`, error);
      throw new Error(`脚本生成失败，请检查API配置或稍后重试`);
    }
  }
